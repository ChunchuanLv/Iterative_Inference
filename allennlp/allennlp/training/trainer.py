...
class Trainer(Registrable):
    ...
    def batch_loss(self, batch: torch.Tensor,  for_training: bool, epoch:int = None) -> torch.Tensor:
        """
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch in train_generator_tqdm:
            ...
            loss = self.batch_loss(batch, for_training=True, epoch=epoch)
            loss.backward()
    ...
    def _description_from_metrics(self, metrics: Dict[str, float]) -> str:
        ...
        return ', '.join(["%s: %.4f" % (name, value) for name, value in
                          sorted( metrics.items(), key=lambda tup: tup[0])  if not name.startswith("_")]) + " ||"
