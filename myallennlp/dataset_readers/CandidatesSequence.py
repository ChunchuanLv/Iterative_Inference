...
class CandidatesSequence(Field[torch.Tensor]):
    """
    ...
    def __init__(self,
        ...
        if skip_indexing:
            ...
            if not all(isinstance(label, str) for label in labels):
                raise ConfigurationError("CandidatesSequence expects  string labels if skip_indexing=False. "
                                         "Found labels: {}".format(labels))
    ...
    def get_padding_lengths(self) -> Dict[str, int]:  # pylint: disable=no-self-use
        ...
        return { "max_senses":len(self.labels)}
    ...
    def as_tensor(self, padding_lengths: Dict[str, int]) -> torch.Tensor:
        ...
        padded_tags = pad_sequence_to_length(self._label_ids, max_senses)
        tensor = torch.LongTensor(padded_tags)
        return tensor

    @overrides
    def empty_field(self):
        return CandidatesSequence([], self._label_namespace, skip_indexing=True)

    def __str__(self) -> str:
        return f"CandidatesSequence with labels: {self.labels} in namespace: '{self._label_namespace}'  max_senses:: '{self._num_labels}'.'"
