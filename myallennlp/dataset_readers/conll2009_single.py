...
from allennlp.data.fields import Field, TextField, SequenceLabelField, MetadataField,AdjacencyField,LabelField,IndexField
from allennlp.data.instance import Instance
from allennlp.data.token_indexers import SingleIdTokenIndexer, TokenIndexer
from allennlp.data.tokenizers import Token
from myallennlp.dataset_readers.CandidatesSequence import CandidatesSequence
...
def parse_sentence(sentence_blob: str) -> Tuple[List[Dict[str, str]], List[Tuple[int, int]], List[str]]:
    ...
    arc_tags = [["_"]* len(annotated_sentence)] * len(predicates_indexes) #[[]]
    for line_idx, line in enumerate(lines):
        for predicate_idx, arg in enumerate(line[len(FIELDS_2009):]):
            if arg != "_":
                arc_tags[predicate_idx][line_idx] = arg
    return annotated_sentence, arc_tags,predicates_indexes
...
@DatasetReader.register("conll2009_en_single")
class Conll2009DatasetReader(DatasetReader):
   ...
   #     print ("reading", file_path)
        with open(file_path) as sdp_file:
            for annotated_sentence, arc_tags,predicates_indexes in lazy_parse(sdp_file.read()):
                # If there are no arc indices, skip this instance.
                if not arc_tags and self.filter and "train" in file_path :
                    continue
                ...
                if "ood" not in file_path:
                    if len(pred_candidates) == 0:
                        yield self.text_to_instance(tokens, -1,pos_tags, dep_tags, ["_"]*len(dep_tags),[], 0, DEFAULT_PADDING_TOKEN,0)
                    else:
                        for i , (predicate_candidate, sense_index,predicate,arc_tag,predicate_index) \
                            in enumerate(zip (pred_candidates, sense_indexes,predicates,arc_tags,predicates_indexes)):
                            yield self.text_to_instance(tokens, predicate_index,pos_tags, dep_tags, arc_tag,predicate_candidate, sense_index, predicate,i)
                else:
                    if len(pred_candidates) == 0:
                        yield self.text_to_instance(tokens, -1,pos_tags, dep_tags, None,[], None, None,0)
                    else:
                        for i , (predicate_candidate, sense_index,predicate,arc_tag,predicate_index) \
                                in enumerate(zip (pred_candidates, sense_indexes,predicates,arc_tags,predicates_indexes)):
                            yield self.text_to_instance(tokens, predicate_index,pos_tags, dep_tags, None,predicate_candidate, None, None,i)
    ...
    def text_to_instance(self, # type: ignore
        ...
        fields["ith"] = LabelField(ith,skip_indexing=True)
        fields["tokens"] = token_field
        fields["predicate_indexes"] = IndexField(predicates_index,token_field)
        ...
        fields["predicate_candidates"] = CandidatesSequence(pred_candidates, label_namespace="predicates")
        if arc_tags is not None :
            fields["arc_tags"] = SequenceLabelField(arc_tags, token_field, label_namespace="tags")
            fields["predicates"] = LabelField(predicates,label_namespace="predicates")
            fields["sense_indexes"] = IndexField(sense_indexes,fields["predicate_candidates"])
