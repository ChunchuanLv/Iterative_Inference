...
@Model.register("guided_transformer_biaffine_parser")
class GuidedTransformerBiaffineDependencyParser(Model):
    """
    ...
    def __init__(self,
                 ...
                 use_transformer_arcs: bool = True,
        ...
        super(GuidedTransformerBiaffineDependencyParser, self).__init__(vocab, regularizer)
        self.use_transformer_arcs = use_transformer_arcs
        self.text_field_embedder = text_field_embedder
        self.encoder = encoder
        self.gumbel_head_t = gumbel_head_t
        encoder_dim = encoder.get_output_dim()
        if not use_transformer_arcs:
        ...
        self._head_sentinel = torch.nn.Parameter(torch.randn([1, 1, encoder.get_input_dim()]))
    ...
    def forward(self,  # type: ignore
        ...
        head_sentinel = self._head_sentinel.expand(batch_size, 1, embedded_text_input.size(2))
        embedded_text_input = torch.cat([head_sentinel, embedded_text_input], 1)

        mask = torch.cat([mask.new_ones(batch_size, 1), mask], 1)


        encoded_text_list, attended_arcs_list = self.encoder(embedded_text_input, mask)
        ...
        if not self.training:
            encoded_text_list, attended_arcs_list = [encoded_text_list[-1]],[attended_arcs_list[-1]]
        for encoded_text,  attended_arcs in zip(encoded_text_list, attended_arcs_list ):
            ...
            if not self.use_transformer_arcs:
