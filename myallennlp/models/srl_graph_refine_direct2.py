...
@Model.register("srl_graph_parser_refine_direct2")
class SRLGraphParserRefineDirect2(Model):
    """
    ...
    def __init__(self,
        ...
        super(SRLGraphParserRefineDirect2, self).__init__(vocab, regularizer)
        ...
        if rep_dim:
            ...
            rep_dim = encoder_dim
        self.arc_tag_refiner =  FeedForward(rep_dim+node_dim+2*num_labels+1, 2,
                                               [hidden_dim,num_labels+1],
                                            [Activation.by_name(activation)(), Activation.by_name("linear")()],dropout=dropout)

        self.predicte_feedforward = FeedForward(rep_dim+num_labels+node_dim, 2,
                [hidden_dim,node_dim],
                [Activation.by_name(activation)(),Activation.by_name("linear")()],dropout=dropout)
    ...
    #    print ("sense_indexes",sense_indexes.size(),sense_indexes)
        ...
        self._labelled_f1(arc_tag_probs, arc_tags + 1, graph_mask.squeeze(-1), sense_probs, predicate_candidates,
                          predicates, None, arc_tag_probs * input_arc_tag_logits, n_iteration=0)
        ...
        for i in range(epoch):
            ...
            predicate_representation = self._dropout(
                self.predicte_feedforward(torch.cat([encoded_text_for_predicate, predicate_emb,all_active_edges], dim=-1)))
            ...
            tag_input_date = torch.cat([arc_tag_probs,all_other_edges,
                                        encoded_text.unsqueeze(2).repeat(1,1,predicate_indexes.size(1),1),predicate_emb.unsqueeze(1).repeat(1,sequence_length,1,1)],dim=-1)
            arc_tag_logits = self.arc_tag_refiner(tag_input_date)
