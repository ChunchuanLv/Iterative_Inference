...
import math
@Model.register("full_var_biaffine_parser")
class FullVariationalBiaffineDependencyParser(Model):
    """
    ...
    def __init__(self,
        ...
        super(FullVariationalBiaffineDependencyParser, self).__init__(vocab, regularizer)
    ...
    def forward(self,  # type: ignore
        ...
        if head_indices is not None and head_tags is not None:
            ...
            arc_nll, tag_nll, pos_nll,kl_div,rec_loss = self._construct_loss(head_tag_representation=head_tag_representation,
                                                    ...
                                                    head_tags=head_tags,
                                                    pos_tags=pos_tags,
                                                    pos_logits=[pos_scores,prior_pos_scores],
                                                    raw_input = raw_embedded_text_input,
                                                    predicted_input = [auto_enc,prior_auto_enc],
                                                    mask=mask)
    ...
    def _construct_loss(self,
                        ...
                        pos_logits: Tuple[torch.Tensor],
                        raw_input : torch.Tensor,
                        predicted_input: Tuple[torch.Tensor],
        ...
        logvars:[``torch.Tensor``], required
            ...
            [(batch_size, sequence_length, dim)]
        logvars:[``torch.Tensor``], required
            A tensor of log varience of prior gaussian
            [(batch_size, sequence_length, dim)]
        ...
        pos_logits,prior_pos_logits = pos_logits
        ...
        if prior_pos_logits is not None:
            normalised_pos_tag_logits = masked_log_softmax(prior_pos_logits, mask.unsqueeze(-1)) * float_mask.unsqueeze(-1)
            normalised_pos_tag_logits = normalised_pos_tag_logits[:, 1:]
            pos_nll = pos_nll - F.nll_loss(normalised_pos_tag_logits.transpose(1, 2), pos_tags, reduction="sum")
        ...
        if self.auto_enc:
            predicted_input,prior_predicted_input = predicted_input
