...
class GraphAutoEncoder2(Seq2SeqEncoder):
    ...
    def __init__(self,
        ...
        self._edge_score = Linear(2 * edge_dim - 1 + node_dim + hidden_dim, score_dim)
    ...
    def get_computation_graph(self,
        ...
        def edge_score_forward(edges, all_edges, nodes):
            ...
            cated = torch.cat([edges, negative_edges, nodes.unsqueeze(1).repeat(1, edges.size(1), 1, 1),
                               argument_rep.unsqueeze(2).repeat(1, 1, edges.size(2), 1)], dim=-1)
            return self._edge_score(cated)
        ...
        def edge_score_backward(grad, score, edges, all_edges, nodes):
            ...
            grad_to_negative_edges = backed_grad[:, :, :, current:current + edges.size(-1) - 1]

            current = current + edges.size(-1) - 1
            ...
            grad_to_nodes = (backed_grad[:, :, :, current:current + nodes.size(-1)] * graph_mask).sum(1)
...
def main():
    ...
    batch_size = 2
    graph_size = 3
    mlpfbb = GraphAutoEncoder2(node_dim, edge_dim, hidden_dim, score_dim)
    mlpfbb.eval()
